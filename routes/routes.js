...
var messagesFunction = require('../functions/messages');
...
module.exports = function(app,io) {
	...
	app.post('/message', function(req, res) {
		...
		var deviceId = req.body.deviceId;
		var body_message = req.body.body;
		var registrationId = req.body.registrationId;

		if(typeof deviceId == 'undefined' || typeof body_message == 'undefined' || typeof registrationId == 'undefined') {
			console.log(constants.error.msg_invalid_param.message);

			res.json(constants.error.msg_invalid_param);

		} else if ( !body_message.trim() || !deviceId.trim() || !registrationId.trim() ) {
			...
			messagesFunction.message(deviceId, body_message, registrationId, function(result){
				device.find({}, function(err, devices) {
					for(var i = 0; i < devices.length; i++) {
						var rId = devices[i].registrationId;
						if(rId != registrationId) {
							sendFunction.sendMessage(body_message, rId,function(result){
